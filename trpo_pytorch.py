"""
Unable to handle continuous control
Source code: https://gist.github.com/elumixor/c16b7bdc38e90aa30c2825d53790d217
"""
import gym
import numpy as np
import torch
...
def w_dist(distribution, distribution_new):
    p0 = distribution.detach().numpy()
    p1 = distribution_new.detach().numpy()
    # p0, p1 are batch histograms
    res = 0
    for i in range(p0.shape[0]):
        res += wasserstein_distance(p0[i], p1[i])
    return res / p0.shape[0]

def kl_div(p, q):
    p = p.detach()
    return (p * (p.log() - q.log())).sum(-1).mean()

def train(epochs=100, num_rollouts=10, render_frequency=None, prob_dist=kl_div):
    mean_total_rewards = []
    global_rollout = 0
...
def update_agent(rollouts, prob_dist=kl_div):
    ...
    KL = prob_dist(distribution, distribution)
    ...
    def criterion(step):
        ...
        with torch.no_grad():
            ...
            KL_new = prob_dist(distribution, distribution_new)
...
x = np.arange(epochs)
for prob_dist in [w_dist, kl_div]:
    print(str(prob_dist))
    ep_rets = train(epochs=epochs, num_rollouts=10, render_frequency=None, prob_dist=prob_dist)
    ax.plot(x, ep_rets, label=str(prob_dist))
